...
2. **Server**: A stateless server that handles direct requests and serves the UI. It can be scaled horizontally by running multiple instances behind a load balancer.
...
User authentication is done via SSO. Currently supported providers are:
* Azure AD
* Google Workspace
...
- **ADMIN_EMAIL**: The email address of the initial admin user.
...
- **SESSION_SECRET**: A secret used to sign session cookies.
- **GOOGLE_CLIENT_ID**: The client ID for Google OAuth.
- **GOOGLE_CLIENT_SECRET**: The client secret for Google OAuth.
- **AZURE_CLIENT_ID**: The client ID for Azure AD OAuth.
- **AZURE_CLIENT_SECRET**: The client secret for Azure AD OAuth.
- **AZURE_TENANT_ID**: The tenant ID for Azure AD OAuth.

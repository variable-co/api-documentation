...
<Note>**Enterpise feature:** [contact us](mailto:hello@variable.co) if you're interested in self-hosting Variable</Note>
...
```mermaid
flowchart TD
    UI(Variable UI) <-->|API Calls| API(API Server)
    API <-->|Database Queries| DB[(Neo4j Server)]
    DB <--> DBD[Neo4j Data]
    DB --> DBL[Neo4j Logs]
    DBI[Neo4j Import] --> DB
    API -->|Upload| PBU@{ shape: docs, label: "PUBLIC_BUCKET_UPLOAD" }
    API -->|Upload| PRU@{ shape: docs, label: "PRIVATE_BUCKET_UPLOAD" }
    PUR@{ shape: docs, label: "PUBLIC_BUCKET_READ" } -->|Read| UI
    PRR@{ shape: docs, label: "PRIVATE_BUCKET_READ" } -->|Read| API
    PBU -->|Malware Scan| MSS(Malware Scanner)

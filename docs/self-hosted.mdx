...
flowchart TD
    ...
    DB --> DBL[Neo4j Logs]
    DBI[Neo4j Import] --> DB
    API -->|Upload| PBU[PUBLIC_BUCKET_UPLOAD]
    API -->|Upload| PRU[PRIVATE_BUCKET_UPLOAD]
    PUR[PUBLIC_BUCKET_READ] -->|Read| UI
    PRR[PRIVATE_BUCKET_READ] -->|Read| API
    PBU -->|Malware Scan| MSS[Malware Scanning Service]
